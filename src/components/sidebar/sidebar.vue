...
export default {
  ...
  methods: {
      ...
      for (const mutation of mutationsList) {
        if (
          ...
          if (newValue == "vertical") {
            let currentPath = this.$route.path.endsWith("/")
    ...
    menuscrollFn() {
      ...
      let menuPosition =
        html.getAttribute("data-menu-position") == "scrollable";
      let header = document.querySelector(".app-header")?.clientHeight || 0;
      window.onscroll = () => {
        if (
          !menuPosition &&
          this.$refs["preventpagejump"] &&
          navLayout &&
  ...
  created() {
    ...
    let currenUrl = this.$route.path.endsWith("/")
      ? this.$route.path.slice(0, -1)
      : this.$route.path;
  ...
  mounted() {
    ...
    if (mainContent) {
      ...
      console.warn("El elemento .main-content no está disponible.");
    }
