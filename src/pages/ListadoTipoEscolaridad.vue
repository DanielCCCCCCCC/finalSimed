<template>
  <div class="row justify-center espaciadoLateral q-py-md">
    <div id="app-container" class="q-mb-xl q-px-xl q-pa-xs">
      <!-- Encabezado y botón para agregar nueva escolaridad -->
      <div class="q-pb-md">
        <div class="row items-center">
          <div class="header-container">
            <h4 class="header-title">Escolaridades Existentes</h4>
            <p class="parrafo">Catálogo de escolaridades del centro médico</p>
          </div>
          <div>
            <q-btn
              label="Nueva Escolaridad"
              flat
              class="btn btn-primary btn-sm btn-wave right-content fsButton fe fe-plus"
              @click="abrirDialogoNuevaEscolaridad"
            />
          </div>
        </div>
      </div>

      <!-- DataGrid para Escolaridades -->
      <DxDataGrid
      ...
      >
        ...
        <DxSummary>
          <DxGroupItem summary-type="count" displayFormat="{0} escolaridades" />
        </DxSummary>
...
// Datos de la escolaridad seleccionada
...
// Función para cargar escolaridades al montar el componente
// const cargarDatos = async () => {
//   try {
//     await escolaridadStore.cargarEscolaridades();
//     console.log("Escolaridades cargadas:", escolaridades.value);
//   } catch (error) {
//     console.error("Error al cargar escolaridades:", error);
//     Notify.create({
//       message: "Error al cargar las escolaridades.",
//       color: "negative",
//       position: "top-right",
//     });
//   }
...
// Abrir el formulario para agregar una nueva escolaridad
const abrirDialogoNuevaEscolaridad = () => {
  isEditMode.value = false;
  resetFormulario();
  mostrarDialogo.value = true;
};

// Abrir el formulario de edición con los datos de la escolaridad seleccionada
const abrirFormularioEdicion = (e) => {
  ...
  if (!data || !data.id) {
    Notify.create({
      type: "negative",
      message: "Error al abrir el modal: la escolaridad no tiene ID.",
      position: "top-right",
    });
    return;
  }
  isEditMode.value = true;
  escolaridadSeleccionada.value = { ...data };
  mostrarDialogo.value = true;
};

// Guardar los cambios del formulario (agregar o actualizar)
const guardarCambios = async () => {
  const { id, descripcion } = escolaridadSeleccionada.value;
  ...
  try {
    if (isEditMode.value) {
      // Actualizar escolaridad
      await escolaridadStore.actualizarEscolaridad(id, descripcion.trim());
      Notify.create({
        type: "positive",
      ...
      // Agregar nueva escolaridad
      await escolaridadStore.agregarEscolaridad(descripcion.trim());
      Notify.create({
        type: "positive",
        message: "Escolaridad agregada con éxito",
        position: "top-right",
      });
    }

    // Recargar escolaridades y cerrar el modal
    await escolaridadStore.cargarEscolaridades();
    cerrarDialogo();
  } catch (error) {
    Notify.create({
      type: "negative",
      message: "Hubo un error al guardar la escolaridad",
      position: "top-right",
    });
...
const eliminarEscolaridad = async (e) => {
  try {
    const escolaridadId = e.row.data.id;
    await escolaridadStore.eliminarEscolaridad(escolaridadId);
    Notify.create({
      type: "positive",
      message: "Escolaridad eliminada con éxito",
      position: "top-right",
    });
    await escolaridadStore.cargarEscolaridades();
  } catch (error) {
    Notify.create({
      type: "negative",
      message: "Error al eliminar la escolaridad",
      position: "top-right",
    });
